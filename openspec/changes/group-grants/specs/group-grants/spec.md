...
Выдача прав Unix-**группе** (sudo/file/limits — наследуют все члены, вкл. эффективно-вложенных
из LDAP) и единая provenance-модель `Created | Adopted`: Census различает объекты, которые сам
создал, от существующих, берёт существующие группы/юзеров под управление (adopt) и отпускает
обратно (release), не разрушая чужое.
