...
- [x] 1.1 Создать `packaging/nfpm.yaml`: метаданные (name `census`, version из env
  ...
  Depends/Requires
- [x] 1.2 В `packaging/nfpm.yaml` описать contents по таблице раскладки design:
...
- [x] 1.3 В `packaging/nfpm.yaml` добавить пустые `type: dir`
  `/etc/census/permissions.d/` и `/etc/census/frameworks.d/` (0755)
- [x] 1.4 Локально провалидировать конфиг: `VERSION=0.1.0 nfpm package -f
...
- [x] 2.1 В `release.yml` после шага `Package` добавить установку pinned-бинаря
  nfpm в `/usr/local/bin` (по образцу установки `taplo` в `ci.yml`: фиксированная
  версия и URL, без apt/компиляции)
- [x] 2.2 Добавить шаг сборки пакетов: `VERSION=${GITHUB_REF_NAME#v} nfpm package
  -f packaging/nfpm.yaml -p deb -t .` и то же с `-p rpm -t .`
- [x] 2.3 Добавить шаг контрольных сумм: `sha256sum` на каждый произведённый
  `census_*.deb` и `census-*.rpm` в одноимённый `.sha256`
- [x] 2.4 Расширить `files:` в шаге `softprops/action-gh-release`: добавить `.deb`,
  `.rpm` и их `.sha256` к уже публикуемому musl-бинарю и его `.sha256`

## 3. Верификация раскладки против бинаря

- [x] 3.1 Распаковать собранный `.deb` (`dpkg-deb -c` / `dpkg-deb -x`) и сверить,
...
- [x] 3.2 Проверить метаданные: версия `.deb`/`.rpm` равна `[package] version` в
...
- [x] 4.1 Удалить superpowers-черновик
