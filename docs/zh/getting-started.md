...
目录内置于 Census 的 `share/permissions/`。用 `--catalog-dir` 指向额外的目录根（可重复；
靠后的根优先）：
...
```sh
census show oper --lang zh --catalog-dir /opt/census/share/permissions
```
...
census plan  --declaration declaration.toml --catalog-dir /opt/census/share/permissions   # 预览增量
