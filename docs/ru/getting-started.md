...
### 4.2.1 Аудит фактических прав устройства

`doctor` проверяет *собственные* инварианты Census; **аудит экспозиции**
проверяет *фоновые* права файловой системы устройства — что субъект уже может
прочитать и записать, независимо от того, что подготовил Census. Он read-only и
тоже выходит с ненулевым кодом при finding'е уровня High, так что заводится в
мониторинг тем же путём:

```sh
sudo census audit fs                       # карта позиции устройства (world-writable, setuid, читаемые секреты, …)
sudo census audit expose --principal oper  # что учётка `oper` реально достаёт, сверх своих grant'ов
```

Запускайте `audit expose` всякий раз, когда создаёте или ужесточаете
ограниченную учётку, чтобы убедиться: фоновая файловая система не выдаёт ей
больше, чем вы задумали. Полное руководство — [audit.md](audit.md).
