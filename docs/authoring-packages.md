...
- `groups` — **доп-группы** роль-учётки. ⚠ Группы должны **существовать** на устройстве
  (Census назначает членство `useradd -G`, базовые группы в текущей версии **не создаёт** —
  см. §12 ограничения).
...
1. **Базовые группы**, перечисленные в `payload.groups` ролей. Census назначает членство,
   но (текущая версия) не создаёт сами группы. Создать заранее (enrollment/образ):
   `groupadd -f atm-operators log-readers`.
...
uid = 9030
```
На устройстве (enrollment): `groupadd -f atm-operators atm-service atm-admin`, разложить
trust.pub. Затем `census plan` → `census apply` (managed: подписать по §9) → `census doctor`.
...
- **Группы не создаются** Census — членство назначается `useradd -G`, базовые группы должны
  существовать заранее (enrollment/образ). Создание отсутствующих групп — возможное будущее.
